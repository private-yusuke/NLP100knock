...
void main() {
	auto neko = readNeko();
	foreach (sentence; neko) {
		foreach (chunk; sentence) {
			if(chunk.morphs.any!(v => v.pos == "動詞")) {
				string[] particles, clauses;
				foreach(ind; chunk.srcs) {
					auto arr = sentence[ind].morphs.filter!(v => v.pos == "助詞").array;
					if(arr) {
						Morph attributed;
						foreach(v; arr) {
							if(v.pos1 == "格助詞") attributed = v;
						}
						if(!attributed) attributed = arr.back;
						particles ~= attributed.surface;
						clauses ~= sentence[ind].surfacesToString();
					}
				}
				if(particles) {
					auto verb = chunk.morphs.filter!(v => v.pos == "動詞").array[0].base;
					writefln("%s\t%s\t%s", verb, particles.join(' '), clauses.join(' '));
				}
			}
		}
	}
