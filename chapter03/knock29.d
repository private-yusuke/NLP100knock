...
void main() {
	string uk = UK();
	auto basicinfo = extractBasicInfo(uk).removeSign.removeBracketsAttribute.removeTags.removeLangs;

	auto m = mapBasicInfo(basicinfo);

	auto filename = m["国旗画像"];
	auto imageinfo = "https://www.mediawiki.org/w/api.php?action=query&format=json&prop=imageinfo&titles=File:%s&iiprop=url".format(filename.map!(i => i == ' ' ? '_' : i).array).get.parseJSON;
